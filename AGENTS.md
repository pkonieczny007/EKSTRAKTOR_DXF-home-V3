...
**Roadmap:** tablica ✅ → prompt ✅ → AGENTS.md ✅ → PLAN.md ✅ → szkielet systemów ✅
...
python zarzadzanie\deploy_skilli.py            # deploy skilli do ~/.Codex/skills + SecondBrain (DRY-RUN; --wykonaj = kopiuje)
...
├── AGENTS.md                  ← ten plik: routing + zasady (jedyne źródło zasad)
├── PLAN.md                    ← etapy wdrożenia 0–6 + kryteria przejścia
├── requirements.txt           ← ezdxf, openpyxl, matplotlib, pyyaml (+shapely w etapie 2)
├── excalidraw/                ← tablice projektowe + kontekst_deploy/ (oryginały Codex v1/v2)
...
- `audyt.py` — porównuje rejestr z rzeczywistością: skill zainstalowany w `~/.Codex/skills`
...
- Źródła skilli w `skills/` w repo (jedno źródło prawdy); deploy = kopia do
  `~/.Codex/skills` + `\\Qnap-energo\baza\.data\SecondBrain\03_Zasoby\Skille`.
...
- Skille: `~/.Codex/skills` + `\\Qnap-energo\baza\.data\SecondBrain\03_Zasoby\Skille`.

## Instrukcje dla Codex w tym projekcie
