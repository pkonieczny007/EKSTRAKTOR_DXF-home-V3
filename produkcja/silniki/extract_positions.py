...
def save_view(cluster, absorbed, bend_inside, scale, layer_name,
    ...
    # bbox ZRODLOWY widoku na papierze (PRZED skalowaniem i wysrodkowaniem) -
    # potrzebny galerii V2 do ramki "skad wycieto" na miniaturze calego rysunku
    sb = cluster["bbox"]
    rep["src_x1"], rep["src_y1"] = round(sb[0], 2), round(sb[1], 2)
    rep["src_x2"], rep["src_y2"] = round(sb[2], 2), round(sb[3], 2)
    ...
    for r in reports:
        ...
        if twin and twin != r["posn"]:
            ...
            if fname:
                ...
                r["file"] = fname
                # lustro nie ma wlasnego widoku - ramka "skad wycieto"
                # pokazuje widok blizniaka, z ktorego powstalo odbicie
                twin_rep = next((t for t in reports if t["posn"] == twin), None)
                if twin_rep:
                    for k in ("src_x1", "src_y1", "src_x2", "src_y2"):
                        r[k] = twin_rep.get(k)
